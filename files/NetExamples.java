...
public class NetExamples {
    ...
    /*
     * Generate buffer with random contents Writes buffer to file using a
     * CompletionHandler to consume the result of each write operation Reads file to
     * EOF to a new buffer using a CompletionHandler to consume the result of each
     * read operation Compares buffer contents
     */
    static void testUsingCompletionHandlers(AsynchronousFileChannel ch) throws IOException {
        ...
        ByteBuffer dst = (rand.nextBoolean()) ? ByteBuffer.allocateDirect(src.capacity())
    ...
    /*
     * Generate buffer with random contents Writes buffer to file, invoking the
     * Future's get method to wait for each write operation to complete Reads file
     * to EOF to a new buffer, invoking the Future's get method to wait for each
     * write operation to complete Compares buffer contents
     */
    static void testUsingWaitOnResult(AsynchronousFileChannel ch) throws IOException {
    ...
    static void testCustomThreadPool(Path file) throws IOException {
        ...
        ExecutorService executor = Executors.newFixedThreadPool(1 + rand.nextInt(10), threadFactory);
    ...
    static void testAsynchronousClose(Path file) throws IOException {
        ...
        // create file
        AsynchronousFileChannel ch = AsynchronousFileChannel.open(file, WRITE, TRUNCATE_EXISTING);
        long size = 0L;
        do {
    ...
    static void testCancel(Path file) throws IOException {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            AsynchronousFileChannel ch = AsynchronousFileChannel.open(file, WRITE, SYNC);
    ...
    static void testTruncate(Path file) throws IOException {
        ...
        // basic tests
        AsynchronousFileChannel ch = AsynchronousFileChannel.open(file, CREATE, WRITE, TRUNCATE_EXISTING);
        try {
    ...
    // given position
    static void writeFully(final AsynchronousFileChannel ch, final ByteBuffer src, long position) {
        final CountDownLatch latch = new CountDownLatch(1);
    ...
    static void readAll(final AsynchronousFileChannel ch, final ByteBuffer dst, long position) {
        final CountDownLatch latch = new CountDownLatch(1);
